...
const jive_mode = (inputs) => {
    ...
    // setup state
    if (nInputs % 2 == 0){
        for (var i = 0; i < nInputs; i+=2){
            stateX.push(inputs[i]);
            stateX.push(inputs[i+1]);
        }
    } else {
        for (var i = 0; i < nInputs - 2; i+=2){
            stateX.push(inputs[i]);
            stateY.push(inputs[i+1]);
        }        
        stateX.push(inputs[nInputs-2]);
        stateX.push(BigInt("0"));
    }
    
    anemoi = anemoiPerm(prime_field, nInputs, numRounds, generator, inverse_generator, BigInt(alpha), inverse_alpha,
